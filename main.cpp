...
void ShowProcessMemoryUsageInfo()
{
	...
	std::cout << std::fixed << setprecision(0) << "内存使用率：" << memoryUsage_K << " KB = " << setprecision(2) << memoryUsage_M << " MB" << std::endl;
}
...
void rundisplay()
{
	...
	while (!_kbhit())
	{
		...
		cout << "CPU利用率:" << right << setw(5) << cpu << "%";
		cout << "      CPU空闲率:" << setw(5) << cpuidle << "%";
		preidleTime = idleTime;
		prekernelTime = kernelTime;
		preuserTime = userTime;
	}
...
int main()
{
	...
	*/   //*******************************
	rundisplay();                                                                   //计时结果显示
	//EmptyWorkingSet(GetCurrentProcess());                      //写入页面文件
	cout << "按任意键退出程序" << endl;
	_getch();
	_getch();
	//closegraph();
